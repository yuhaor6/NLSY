...
* Start log
log using "EDA_DeepDive_log.txt", replace text
...
capture confirm file "merged_data_with_occind.dta"
if _rc != 0 {
    di as error "ERROR: merged_data_with_occind.dta not found!"
    di as error ""
    di as error "You need to add this line to Data_process.do after line 1048:"
    di as error "    save merged_data_with_occind.dta, replace"
    di as error ""
    di as error "Then re-run Data_process.do before running this EDA."
    exit 601
}
...
histogram n_obs, bin(30) ///
    ...
    note("N = `ncells' cells")
graph export "hist_obs_per_cell.png", replace
...
histogram n_individuals, bin(30) ///
    ...
    note("N = `ncells' cells")
graph export "hist_individuals_per_cell.png", replace
...
graph box log_pwages, over(cell_str, sort(1) descending label(angle(45) labsize(small))) ///
    ...
    ytitle("Log Wages")
graph export "boxplot_wages_top_cells.png", replace
...
graph export "wage_exp_profile_pooled.png", replace
...
graph export "wage_exp_by_occupation.png", replace
...
graph export "wage_exp_by_industry.png", replace
...
histogram beta_cumhrs, bin(20) ///
    ...
    xline(0, lcolor(red))
graph export "hist_cumhrs_coef.png", replace
...
twoway (rcap ci_lo ci_hi rank, horizontal lcolor(gray)) ///
       ...
       legend(off)
graph export "forest_cumhrs_returns.png", replace
...
graph export "hist_person_mean_wage.png", replace
...
graph export "avg_wage_age_profile.png", replace
...
twoway connected mean_log_wage event_time, ///
       ...
       xlabel(-3(1)3)
graph export "event_study_cell_switch.png", replace
...
graph export "density_overlay_cells.png", replace
