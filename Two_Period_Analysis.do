...
log using "two_period_analysis_log.txt", replace text
...
capture estimates table annual_age1 annual_age2 annual_age3, ///
    keep(log_ntr_change) b(%9.3f) se(%9.3f) stats(N) ///
    title("Annual Period: Under 25 | 25-29 | 30+")

di ""
di "BIENNIAL PERIOD BY AGE:"
capture estimates table biennial_age1 biennial_age2 biennial_age3, ///
    keep(log_ntr_change) b(%9.3f) se(%9.3f) stats(N) ///
...
capture estimates table annual_pre_erta annual_erta annual_tra86 annual_post_tra86, ///
    keep(log_ntr_change) b(%9.3f) se(%9.3f) stats(N) ///
    title("Annual: Pre-ERTA | ERTA | TRA86 | Post-TRA86")

di ""
di "BIENNIAL PERIOD TAX REFORMS:"
capture estimates table biennial_pre_egtrra biennial_bush biennial_recession biennial_tcja, ///
    keep(log_ntr_change) b(%9.3f) se(%9.3f) stats(N) ///
...
capture estimates table annual_inc1 annual_inc2 annual_inc3, ///
    keep(log_ntr_change) b(%9.3f) se(%9.3f) stats(N) ///
    title("Annual: $10-50K | $50-100K | $100K+")

di ""
di "BIENNIAL PERIOD BY INCOME:"
capture estimates table biennial_inc1 biennial_inc2 biennial_inc3, ///
    keep(log_ntr_change) b(%9.3f) se(%9.3f) stats(N) ///
...
save "two_period_summary.dta", replace
export delimited using "two_period_summary.csv", replace
